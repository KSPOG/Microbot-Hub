...
## v0.2.3 reliability fix

Incident handling deliberately runs on a background worker so Git, Codex and compilation cannot block the RuneLite Client thread. Some incident evidence, however, must be read through RuneLite's `Client` API and those calls are client-thread-only.

v0.2.3 passes the coordinator a thread-aware `Client` proxy. Calls already made on RuneLite's Client thread are executed directly. Calls made by the repair worker are synchronously marshalled through `ClientThread.invoke(...)`, then their result is returned to the worker. This prevents incident capture from failing with `IllegalStateException: must be called on client thread` while retaining background execution for expensive repair work.
...
The built-in Codex / ChatGPT option invokes the local Codex CLI non-interactively. Authentication is provided by the local Codex CLI login using your ChatGPT account. The generated repair prompt is supplied over stdin and the managed repository is the only writable workspace.
...
Leave the Codex model setting blank to use the model selected by your local Codex configuration/default.
