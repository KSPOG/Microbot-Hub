...
import net.runelite.client.plugins.microbot.kspaccountbuilder.skills.ge.Buy;
...
public class WoodcuttingScript {
    ...
    public boolean hasRequiredTools() {
        return getUsableAxeCount() >= ItemReqs.MIN_AXE_COUNT;
    }
    ...
    private boolean ensureBestAxeForCurrentLevel(int woodcuttingLevel, int attackLevel) {
        int targetAxeId = getBestAxeIdByWoodcuttingLevel(woodcuttingLevel);
        if (targetAxeId == -1) {
            return false;
        }

        if (!hasAxe(targetAxeId)) {
            ...
            if (bestAvailableAxe == -1) {
                log.info("No eligible axe available. Walking to GE to buy axes.");
                if (!Buy.buyMissingAxes()) {
                    return false;
                }
                if (!Rs2Bank.walkToBank() || !Rs2Bank.openBank()) {
                    return false;
                }
                bestAvailableAxe = getBestAvailableAxeId(woodcuttingLevel);
                if (bestAvailableAxe == -1) {
                    return false;
                }
            }
            ...
            targetAxeId = bestAvailableAxe;
        }

        if (canEquip(targetAxeId, attackLevel)
                && Rs2Inventory.hasItem(targetAxeId)
                && !Rs2Equipment.isWearing(targetAxeId)) {
            Rs2Inventory.interact(targetAxeId, "Wield");
        }

        return hasAxe(targetAxeId);
    }

    private int getBestAvailableAxeId(int woodcuttingLevel) {
        for (int axeId : AXE_IDS_DESC) {
            if (woodcuttingLevel >= getWoodcuttingRequirement(axeId) && (hasAxe(axeId) || Rs2Bank.hasItem(axeId))) {
                return axeId;
            }
        }
        return -1;
    ...
    private WorldArea getBestAreaForLevel(int woodcuttingLevel, WorldPoint playerLocation) {
        if (woodcuttingLevel >= TreeLevels.YEW) return Areas.YEW;
        if (woodcuttingLevel >= TreeLevels.WILLOW) return Areas.WILLOW;
        if (woodcuttingLevel >= TreeLevels.OAK) return getClosestOakArea(playerLocation);
        return Areas.TREES;
    }
